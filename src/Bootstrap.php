...
use DekodeGutenbergBlocks\Common\Traits\Requester;
use DekodeGutenbergBlocks\Common\Utils\Errors;
use DekodeGutenbergBlocks\Config\Classes;
use DekodeGutenbergBlocks\Config\I18n;
use DekodeGutenbergBlocks\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'dekode_gutenberg_blocks_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'dekode_gutenberg_blocks_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'dekode-gutenberg-blocks' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Dekode Gutenberg Blocks classes are not being loaded by Composer\'s Autoloader' ),
